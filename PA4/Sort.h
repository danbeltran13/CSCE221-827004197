...
{
	//loop ends at 1 before the end of the vector so that second loop does not go out of bounds
    for(int i =0; i < a.size()-1;++i){
			
		Comparable min =a[i+1];
		int index =i+1;
		//find min starting from next element
...
{
	if(a.size()>=16){
		//Dives vector into 4 parts, last part is bigger than rest
		int subSize = a.size()/4;
		int remainder = a.size()%4;
		int i;
		//call first 3 sublists
		for(i=0;i<a.size()-subSize-remainder; i+= subSize){
			quicksort(a,i,i+subSize-1);
		}
		//final sublist,
		quicksort(a,i+1,a.size()-1);
